...
def test_overlap_screening_631g_gbs():
    ...
    # tolerance for overlap is by default 1E-20. To change add eps = X as the end argument
    ...
    # tolerance for overlap is by default 1e-20. To change add eps = X to make_contractions()
    contraction[0].create_overlap_mask(contraction)
...
def test_overlap_screening_vs_without_screening():
    ...
    assert np.allclose(overlaps, overlaps_no_screen, rtol=1.0e-15, atol=1.0e-18)

    # Test that screened and un-screened overlaps are DIFFERENT [use large eps for (small) cutoff]
    contraction = make_contractions(
    ...
    # tolerance for overlap is by default 1e-20. To change add eps = X as the end argument
    for item in contraction:
        item.create_overlap_mask(contraction, eps=1.0e-10)

    overlaps = overlap_integral(contraction)
    overlaps_no_screen = overlap_integral(contraction_without_screen)

    assert not np.allclose(overlaps, overlaps_no_screen, rtol=1.0e-15, atol=1.0e-18)
    ...
    # Test that screened and unscreened overlaps are DIFFERENT [use large eps for (small) cutoff]
    contraction = make_contractions(
        basis_dict, ["C", "C", "C"], np.array([[0, 0, 0], [0, 0, 11], [1, 1, 20]])
    )
    contraction_without_screen = make_contractions(
